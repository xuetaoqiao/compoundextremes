...
# 5) Run paired Wilcoxon tests: compound vs each individual (alternative = "two.sided")
# ============================================================
...
# Paired Wilcoxon tests: compound vs ind1 and compound vs ind2 (alternative = "two.sided") 
wilcox_two <- function(dat_wide, metric) {
  ...
  list(
    p_ind1 = wilcox.test(com, ind1, paired = TRUE, alternative = "two.sided")$p.value,
    p_ind2 = wilcox.test(com, ind2, paired = TRUE, alternative = "two.sided")$p.value
  )
}
...
panels <- list(
  a = list(
    ...
    lab_ind2 = "Heat alone",
    x_lab = expression(Resistance ~ (log[2]*italic(R)[s])),
  ...
  c = list(
    ...
    lab_ind1 = "Moisture excess alone",
    lab_ind2 = "Heat alone",
    x_lab = expression(Recovery ~ (log[2]*italic(R)[c])),
    y_limits = c(0, 500), y_breaks = seq(0, 500, 100),
    legend_pos = c(0.29, 0.90), mean_yend = 420
  ),
  d = list(
