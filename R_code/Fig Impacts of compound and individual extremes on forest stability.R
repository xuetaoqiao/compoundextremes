...
data_ceshi_1 = data_com_pup_tup %>%
  ...
  mutate(
    delt_Rs_pup = log2(Rs_sin + 1) - log2(Rs_com + 1)  # Calculate log2 difference for Rs
  ) %>%
  mutate(
...
data_ceshi_3 = data_com_pdown_tup %>%
  ...
  mutate(
    delt_Rs_pdown = log2(Rs_sin + 1) - log2(Rs_com + 1)  # Calculate log2 difference for Rs
  ) %>%
  mutate(
...
data_ceshi_4 = data_com_pdown_tup %>%
  inner_join(data_sin_tup, by = "plot_ID") %>%  # Merge datasets by plot_ID
  mutate(
    delt_Rs_tup = log2(Rs_sin + 1) - log2(Rs_com + 1)  # Calculate log2 difference for Rs
  ) %>%
  mutate(
    factor_tup  = case_when(
      Rs_sin > Rs_com ~ "small_com",  # If sin Rs > com Rs
...
t.test(log2(data_Rs$Rs_com + 1), log2(data_Rs$Rs_sin + 1))

# Calculate mean and standard deviation for Rs_sin and Rs_com
Rs_top = paste(
  format(round(mean(log2(data_Rs$Rs_sin + 1)), 3), nsmall = 3),
  " ± ", 
  format(round(sd(log2(data_Rs$Rs_sin + 1)), 3), nsmall = 3), 
  sep =''
) 
Rs_bottom = paste(
  format(round(mean(log2(data_Rs$Rs_com + 1)), 3), nsmall = 3), 
  " ± ", 
  format(round(sd(log2(data_Rs$Rs_com + 1)), 3), nsmall = 3), 
  sep =''
) 
...
p1_1 = ggplot(data_Rs, aes(x = log2(Rs_com + 1), y = log2(Rs_sin + 1))) +
  ...
  # Customize y-axis
  scale_y_continuous(
    name = expression(atop(Resistance~(ln(italic(R)[s] + 1)), paste(under~individual~extremes))),
    breaks = seq(-3, 21, 6), 
    limits = c(-3.5, 21.5),
    expand = c(0, 0)
  ) +
  # Customize x-axis
  scale_x_continuous(
    name = expression(atop(Resistance~(ln(italic(R)[s] + 1)), paste(under~compound~extremes))),
    breaks = seq(-3, 21, 6), 
    limits = c(-3.5, 21.5),
    expand = c(0, 0)
  ) +
