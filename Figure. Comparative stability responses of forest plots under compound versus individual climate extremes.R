...
p1_1 <- plot_bin2d(
  df_xy = df_rs,
  bins  = 195,
  xlab_expr = expression(atop(Resistance~(log[2]*italic(R)[s]),
                              paste(under~compound~extremes))),
  ylab_expr = expression(atop(Resistance~(log[2]*italic(R)[s]),
...
p2_1 <- plot_bin2d(
  df_xy = df_le,
  bins  = 175,
  xlab_expr = expression(atop(Recovery~(log[2]*italic(R)[c]),
                              paste(under~compound~extremes))),
  ylab_expr = expression(atop(Recovery~(log[2]*italic(R)[c]),
                              paste(under~individual~extremes))),
  p_value = p_le
)
p2_1
